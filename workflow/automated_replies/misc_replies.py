...
async def handle_voice_message(db, client, user, ticket, lang):
    user_id = user.get("user_id")
    await client.send_message(user_id, "Can you please send text instead of a voice message?")
    await asyncio.sleep(random.uniform(6, 8))
    await client.send_message(user_id, "My phones audio doesn't work")

async def handle_thanks(db, client, user, ticket, lang):
    user_id = user.get("user_id")
